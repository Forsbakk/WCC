...
function Get-Points {
    ...
    foreach ($p in $predictionFiles) {
        ...
        if ($qfwinner -eq $null) {}
            ...
            $qfpoints = ((Compare-Object $pqfwinner $qfwinner -ExcludeDifferent -IncludeEqual).Count * 4)
        }

        if ($sfwinner -eq $null) {}
        else {
            $psfwinner = $prediction.M61Win, $prediction.M62Win
            $sfpoints = ((Compare-Object $psfwinner $sfwinner -ExcludeDifferent -IncludeEqual).Count * 8)
        }
